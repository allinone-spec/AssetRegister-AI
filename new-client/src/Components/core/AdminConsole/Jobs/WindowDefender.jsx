...
const WindowDefenderForm = ({ routeName, editJob, setEditJob }) => {
  ...
  const onSubmit = async (data) => {
    try {
      setSaveLoading(true);
      setShowRunJob(false);
      const response = await postDataRequest("/windowDefender/saveJob", data);
  ...
  const handleUpdateJob = async (data) => {
    ...
    try {
      ...
      const response = await patchRequest(
        `/windowDefender/${data.jobName}/updatejob`,
        UpdatePayload,
      );
  ...
  const handleRunjob = async (data) => {
    ...
    try {
      ...
      // const response = await postDataRequest(`/windowDefender/runJob`, payload);
      if (response?.status === 200) {
        toast.success("Successfully Submitted");
        setTitleResponse(response.data);
      }
...
export default WindowDefenderForm;
