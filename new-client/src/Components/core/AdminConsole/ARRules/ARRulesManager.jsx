...
const ARRulesConfig = ({ routeName, job, editJobHandler }) => {
  ...
  const tableName = job ? job?.DCTableName : searchParams.get("table-name");
  const jobName = job ? job?.jobName : searchParams.get("jobName");
  const jobId = job ? job.id : searchParams.get("jobId");
  ...
  useEffect(() => {
    ...
    if (location.state?.job?.ruleId || job?.ruleId) {
      setDataLoading(true);
      getRequest(
        `/rules/get?objectId=${location?.state?.job?.object || job?.object}`,
      )
        .then((res) => {
          const data = res.data
            .filter(
              (v) =>
                v.ruleId == location.state?.job?.ruleId ||
                v.ruleId == job?.ruleId,
            )
        ...
        >
          ...
          <DndProvider backend={HTML5Backend}>
            {queries.map((query, queryIndex) => (
              <Box key={queryIndex} sx={{ mb: job ? 1 : 4 }}>
                <QueryItem
                  key={query.id}
                  id={query.id}
                  index={queryIndex}
                ...
                <FormControl fullWidth sx={{ mb: job ? 0 : 2 }}>
                  <InputLabel>Select Rule</InputLabel>
                  <div className={`${job ? "flex mb-3" : "w-full"}`}>
                    <Select
                      value={query.rule}
                      label="Select Rule"
                      className="w-[93%]"
