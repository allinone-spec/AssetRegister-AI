...
const FileList = ({ routeName, fileListData, setEditJob, setOpenDrawer }) => {
  const { folderId } = fileListData
  ...
  const { folderName, id } = fileListData
    ? fileListData
    : location?.state || {};
  ...
  const fetchSourceData = useCallback(async (folderId) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    if (folderId) fetchSourceData(folderId);
    return () => {
      dispatch(setFilters([]));
    };
  }, [folderId]);
    ...
    <PageLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-3">
        <div className="flex gap-3 sm:gap-6 justify-start">
          <div className="flex items-center gap-2">
            ...
            <div className="font-extrabold text-2xl">{folderName}</div>
      ...
      {filterData.length ? (
        <DataTable
          ...
          cardNavigateEditHandler={navigateViewColumn}
          tableView={view}
          isDrawer
          cardFiled={{
            id: "id",
