...
export const Register = ({ routeName, openDrawer, setOpenDrawer }) => {
  ...
  const column = sourceData.length
    ? Object.keys(...sourceData).map((key) => ({
        ...
        enableLinkHandler: (val) => {
          ...
          setOpenDrawer({
            columnName: key,
            primaryValue: val[primaryKeyVal.key],
          });
        },
      }))
    : [];
    ...
    <PageLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-3">
        <div className="flex items-center gap-2">
          <div className="font-extrabold text-2xl">Detailed</div>
      ...
      {selectedValue ? (
        <DataTable
          ...
          openDrawerId={openDrawer?.["Computer Name"]}
          cardFiled={{
            id: "Computer Name",
            title: "Computer Name",
            subTitle1: "OperatingSystem",
            subTitle2: "Model",
            status: "",
            assets: "No of Core",
          }}
