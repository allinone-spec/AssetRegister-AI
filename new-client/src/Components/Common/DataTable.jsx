...
import { Spark, STATUS_META, StatusPill, TYPE_ICONS } from "./sideDrawer/utils";
import { EmailSideDrawer } from "./EmailSideDrawer";
import {
  analyzeDataset,
  chatWithData,
  clearChatSession,
...
const ColumnOptionsDropdown = ({
  ...
  return (
    <div
      ref={dropdownRef}
      className="absolute bg-surface border rounded-lg shadow-lg py-2 z-50 min-w-48"
      style={{
        top: position.top,
...
const ColumnFilterDropdown = ({
  ...
  // Use external filters if provided (for drawer isolation), otherwise Redux
  const reduxAdvancedFilters = useSelector(
    (state) => state.advancedFilter.filters,
  );
  const advancedFilters = externalFilters || reduxAdvancedFilters;
  ...
  return (
    <div
      ref={dropdownRef}
      className={`absolute bg-surface border rounded-lg shadow-lg py-3 px-4 z-50 min-w-80 max-h-96 overflow-y-auto`}
      style={{
        top: position.top,
...
const DraggableTableHeader = ({
  ...
  return (
    <th
      ref={setNodeRef}
      style={style}
      className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider relative bg-surface"
...
const SortableRow = ({ row, children }) => {
  ...
  return (
    <tr
      ref={setNodeRef}
      style={style}
      className={`border-b bg-surface hover:bg-input-bg border-page-bg ${
...
const LinkCell = ({ getValue, linkHandler, row, hideCell }) => {
  ...
  return (
    <span
      className="cursor-pointer text-[16px] text-blue-600 dark:text-blue-500"
...
const DataTable = ({
  ...
  const [isAdvanceFilterModalOpen, setIsAdvanceFilterModalOpen] =
    useState(false);
  const [viewAddedFilter, setViewAddedFilter] = useState(null);
  ...
  const lastFilterConditionsRef = useRef(null);

  // Local state for drawer filters to avoid Redux conflicts
  const drawerInitRef = useRef(false);
  const [drawerFilters, setDrawerFilters] = useState(() => {
    // Initialize from saveFilters.filterExpression on mount so filters survive drawer close/reopen
    if (isDrawer && saveFilters?.filterExpression?.conditions?.length) {
      drawerInitRef.current = true;
      return saveFilters.filterExpression.conditions.map((c) => ({
        column: c.field,
        field: c.field,
        condition: c.operator,
        value: c.value,
        operator: saveFilters.filterExpression.logic || "AND",
        table: c.table || "",
        table2: c.table2 || "",
        field2: c.field2 || "",
      }));
  ...
  const [drawerActiveColumns, setDrawerActiveColumns] = useState(() => {
    if (isDrawer && saveFilters?.filterExpression?.conditions?.length) {
      return [
        ...new Set(saveFilters.filterExpression.conditions.map((c) => c.field)),
      ];
    }
    return [];
  });

  const { filters: reduxFilters, activeColumns: reduxActiveColumns } =
    useSelector((state) => state.advancedFilter);

  // Use local state for drawer, Redux state for main page
  const myData = isDrawer ? drawerFilters : reduxFilters;
  const activeColumns = isDrawer ? drawerActiveColumns : reduxActiveColumns;

  // Helper functions for filter management
  const clearFilters = () => {
    if (isDrawer) {
      if (drawerFilters.length > 0) setDrawerFilters([]);
      if (drawerActiveColumns.length > 0) setDrawerActiveColumns([]);
    } else {
      if (reduxFilters.length > 0) dispatch(setFilters([]));
    }
  };

  const removeColumnFilter = (columnId) => {
    if (isDrawer) {
      setDrawerFilters((prev) => prev.filter((f) => f.column !== columnId));
      setDrawerActiveColumns((prev) => {
        const hasOtherFilters = drawerFilters.some(
          (f) => f.column === columnId && drawerFilters.length > 1,
        );
        return hasOtherFilters ? prev : prev.filter((col) => col !== columnId);
      });
    } else {
  ...
  const aiInsightDrillDown = useMemo(() => {
    if (!isAiSupportedPage || !Array.isArray(tableData) || tableData.length === 0)
      return null;
    const total = tableData.length;
    const rows =
      total > AI_INSIGHT_DRILL_MAX
        ? tableData.slice(0, AI_INSIGHT_DRILL_MAX)
        : tableData;
    const loadAllRows =
      typeof fetchAllSourceData === "function"
        ? async () => {
            try {
              ...
              return all.length > AI_INSIGHT_DRILL_MAX
                ? all.slice(0, AI_INSIGHT_DRILL_MAX)
                : all;
            } catch {
              return [];
            }
  ...
  // DnD Sensors
  // Restore drawer filters from saveFilters if they weren't available at mount time
  useEffect(() => {
    if (
      isDrawer &&
      !drawerInitRef.current &&
      drawerFilters.length === 0 &&
      saveFilters?.filterExpression?.conditions?.length
    ) {
      const conditions = saveFilters.filterExpression.conditions;
      const restored = conditions.map((c) => ({
        column: c.field,
        field: c.field,
        condition: c.operator,
        value: c.value,
      ...
      setDrawerFilters(restored);
      setDrawerActiveColumns([...new Set(restored.map((f) => f.column))]);
      drawerInitRef.current = true;
    }
  }, [isDrawer, saveFilters?.filterExpression?.conditions]);
      ...
      fetchAiModels()
        .then((res) => {
          ...
          setSelectedModelId((prev) =>
            resolveAiModelSelection(prev, list, res?.defaultChatModelId || ""),
          );
        })
        .catch(() => setAiModels([]));
    }
  ...
  useEffect(() => {
    ...
    return () => {
      if (!setSaveFilters && !isSelectedObject) {
        if (isDrawer) {
          setDrawerFilters([]);
          setDrawerActiveColumns([]);
        } else {
          dispatch(setFilters([]));
        }
      }
  ...
  const handleGroupBy = () => {
    ...
    if (setSaveFilters) {
      // When ungrouping, clear tableData immediately to avoid processing full dataset
      // Parent will provide paged data via API
      if (newGrouping.length === 0 && grouping.length > 0) {
        setTableData([]);
      }
      setFiltering("");
      setSearchValue("");
  ...
  useEffect(() => {
    ...
    if (hadGrouping && !hasGrouping && setSaveFilters) {
      setColumnFilters({});
      // Don't call setTableData(data) here — it would load the full grouped dataset
      // (e.g. 17000 rows) into the ungrouped table causing browser freeze.
      // handleGroupBy already cleared tableData; parent will provide paged data via API.
    }
  ...
  const handleApplyColumnFilter = (
    ...
    // Reset pagination for both manual and local pagination
    if (pagination) {
      setManualPagination((prev) => ({
        ...prev,
        pageIndex: 0,
    ...
    if (filterFn) {
      ...
      // Add to local drawer state or Redux store
      if (isDrawer) {
      ...
      // Remove from store (Redux or local state)
      removeColumnFilter(columnId);
    }

    // Only call onPaginationChange for manual pagination
  ...
  useEffect(() => {
    ...
    setColumnFilters((prev) => {
      ...
      const nextKeys = Object.keys(advancedColumnFns);
      if (prevKeys.length === 0 && nextKeys.length === 0) return prev;
      return advancedColumnFns;
    });

    // Build new conditions to compare with last sent
    const newConditions = myData
    ...
    const conditionsKey = JSON.stringify(newConditions);

    // For drawer: don't clear saved filterExpression on initial mount when drawerFilters
    // hasn't been restored yet (saveFilters may not have been available at useState time)
    if (isDrawer && myData.length === 0 && !drawerInitRef.current) {
      // Skip - preserve existing saveFilters until drawer filters are restored
    } else {
      if (isDrawer && myData.length > 0) {
        drawerInitRef.current = true;
      }
      // Only call setSaveFilters when conditions actually changed to prevent infinite loops
      if (lastFilterConditionsRef.current !== conditionsKey) {
        lastFilterConditionsRef.current = conditionsKey;
        setSaveFilters &&
          setSaveFilters((pre) => ({
            ...pre,
            filterExpression: {
        ...
        cell: ({ row }) => {
          return (
            <div style={{ display: "flex", alignItems: "center" }}>
              {path && (
                <>
                  {permissionList?.includes("AR Mapping") &&
                    permissionDetails["AR Mapping"]?.hasWriteOnly && (
                      <button
                        onClick={() =>
                          setOpenDrawer &&
                          setOpenDrawer({
                            ...row.original,
                            mode: "ar-mapping",
                            // Ensure required properties for AR Mapping
                            ACTableName: row.original.ACTableName || row.original.DCTableName || row.original.tableName,
                            DCTableName: row.original.DCTableName || row.original.ACTableName || row.original.tableName,
                            filterId: row.original.filterId || row.original.id
                          })
                        }
                  ...
                  {permissionList?.includes("AR Rules") &&
                    permissionDetails["AR Rules"]?.hasWriteOnly && (
                      <button
                        className="flex items-center gap-1 text-blue-600 hover:underline text-sm"
                        onClick={() =>
                          setOpenDrawer &&
                          setOpenDrawer({
                            ...row.original,
                            mode: "ar-rules",
                            // Ensure required properties for AR Rules
                            DCTableName: row.original.DCTableName || row.original.ACTableName || row.original.tableName,
                            jobName: row.original.jobName,
                            id: row.original.id || row.original.ruleId,
                            ruleId: row.original.ruleId || row.original.id
                          })
                        }
                      >
                        <IoNavigate />
                  ...
                  <div className="relative group">
                    <IconButton className="relative">
                      ...
                      <div
                        ...
                        className="--accent absolute bottom-8 left-1/2 transform -translate-x-1/2 mb-2 text-white p-2 rounded-md text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-50"
                      >
                        Copy and New
                      </div>
                    </IconButton>
              ...
              {editActionHandler && (
                <button
                  ...
                  style={{
                    border: "1.5px solid rgba(111,47,225,.12)",
                  }}
                  className="px-1 rounded-lg h-7 w-7 mr-2"
                >
                  ✎
                </button>
              )}
              ...
              {row.original[deleteId] || deleteId == true ? (
                <button
                  ...
                  className="px-1 rounded-lg h-7 w-7"
                  style={{
                    color: "#dc2626",
                    border: "1.5px solid rgba(111,47,225,.12)",
                  }}
  ...
  const applyKpiActionOverrides = (result, actionMapOverride = null) => {
    ...
    Object.values(actionMap).forEach((meta) => {
      ...
      if (!title) return;
      const exists = nextKpis.some(
        (k) => normalizeKpiTitle(k?.title) === normalizeKpiTitle(title),
      );
      if (!exists) {
        nextKpis.push({
          title,
          value: "Requested",
          description:
            "Added from chat action. Refresh after new data updates for computed value.",
        });
      }
    });
  ...
  const handleToggleKpiLine = (kpiTitle, checked) => {
    ...
    if (checked) {
      setQueuedKpiRequests((prev) => {
        if (
          prev.some((x) => normalizeKpiTitle(x) === normalizeKpiTitle(title))
        )
          return prev;
        return [...prev, title];
      });
    ...
    setQueuedKpiRequests((prev) =>
      prev.filter((x) => normalizeKpiTitle(x) !== normalizeKpiTitle(title)),
    );
  ...
  const handleInsightFeedback = async (
    ...
    try {
      ...
      if (hideForSession) {
        ...
        toast.success(
          "Thanks — we will emphasize deeper follow-ons in this area on the next refresh.",
        );
      }
      if (insightType === "kpi") {
  ...
  const handleAddChatAnswerToInsights = async (message) => {
    ...
    setAiResult((prev) => {
      ...
      const current = Array.isArray(safePrev.totalInsights)
        ? safePrev.totalInsights
        : [];
      if (
        current.some((x) => String(x?.chatMessageId || "") === message.messageId)
      )
        return safePrev;
      return {
        ...safePrev,
  ...
  const userWantsInsightsRefresh = (text) => {
    ...
    return /\b(refresh|re-analyze|reanalyze|update (the )?dashboard|update (the )?insights|run analysis again|re-run analysis)\b/.test(
      t,
    );
  };
  ...
  return (
    <div className={`space-y-2 px-2 ${className}`}>
      ...
      <div className="sm:flex gap-5 justify-between items-center ms-3">
        {enableFilter && (
          <main className="flex items-center gap-2">
            ...
            {enableToSearch && (
              <div
                className={`relative transition-all self-end rounded-full sm:w-72 w-60
                  ${isDrawer ? "bg-input-bg" : "bg-surface"}
                `}
              >
                <div
                ...
                <input
                  ...
                  name="search"
                  value={filtering}
                  className={`w-full py-1 pl-10 sm:pr-4 text-gray-700 outline-none transition-all border rounded-full bg-transparent border-transparent text-sm`}
                  onFocus={handleFocus}
        ...
        <div className="flex gap-5 sm:items-center mx-3 flex-col sm:flex-row mt-2">
          {/* {enableToSearch && (
            <div
                ...
                <div className="relative">
                  ...
                  >
                    <div className="relative group">
                      ...
                      <div
                        // style={{ backgroundColor }}
                        className="bg-accent absolute bottom-full left-1/2 transform -translate-x-1/2 text-white p-2 rounded-md text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                      >
                        Column
                      </div>
                    </div>
                  ...
                  {isColumnDropdownOpen && (
                    <div className="absolute left-[-50px] mt-1 border rounded shadow-lg p-3 z-20 min-w-48 bg-surface h-72 overflow-scroll">
                      {table
                        .getAllLeafColumns()
                        .filter(
                          (v) =>
              ...
              enableCreateDashboard && (
                <div className="relative group">
                  <MdOutlineDashboardCustomize
                    ...
                    onClick={() => {
                      if (setOpenDrawer) {
                        setOpenDrawer("create-dashboard");
                        // setOpenDrawer({
                        //   mode: "create-dashboard",
                        //   folderData: dashboardData,
                        //   routeName: routeName,
                        // });
                      }
                    }}
              ...
              (permissionList?.includes(routeName) || isDownload) && (
                <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    {isModalOpen && (
                      <div className="absolute top-full right-0 z-20  mt-2 bg-surface border rounded-md shadow-md">
                        <ul className="py-2">
                          {options.map((option) => (
                            <li
                              key={option}
                  ...
                  {showAnalyzeButton && (
                    <div className="relative group">
                      <div className="rounded-xl p-1.5 ring-1 ring-border-theme bg-surface/95 shadow-sm hover:ring-[rgba(var(--accent-rgb),0.35)] hover:bg-accent-dim transition-all">
                        <FiZap
                      ...
                      <div className="bg-accent absolute bottom-full left-1/2 transform -translate-x-1/2 text-white p-2 rounded-md text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        AI insights & chat
                      </div>
                    </div>
                  )}
      ...
      {isLoading ? (
        ...
        <div
          className={`${className === "relative" ? "" : "z-10"} relative`}
        ...
        >
          ...
          <ColumnFilterDropdown
            ...
            externalFilters={isDrawer ? drawerFilters : undefined}
          />
          ...
          >
            <div className="overflow-x-auto rounded-tl-lg rounded-tr-lg">
              {tableView === "table" ? (
                <table className="w-full">
                  <thead className="bg-gray-100 sticky top-0 z-10">
                    {table.getHeaderGroups().map((headerGroup) => (
                      ...
                      >
                        {enableRowOrdering && <th className="bg-surface" />}
                        <SortableContext
                          items={columnOrder}
                  ...
                  <tbody className="relative z-[9px] text-sm">
                    <SortableContext
                      items={
                        enableRowOrdering && !grouping.length
                          ? tableData.map((_, index) => index.toString())
                          : []
                      }
                        ...
                        .map((row, index) => {
                          if (enableRowOrdering) {
                            return (
                              <SortableRow
                                key={row.id}
                                activeId={openDrawerId == row.original.id}
                              ...
                              >
                                {row.getVisibleCells().map((cell) => (
                                  <td
                                    onClick={() => {
                                      if (
                                        openDrawerId == cell.row.id &&
                                        setOpenDrawer &&
                                        !["action", "select", "sno"].includes(
                                          cell.column.id,
                                        )
                                      )
                                        setOpenDrawer(null);
                                        ...
                                        setOpenDrawer(cell.row.original);
                                    }}
                                    key={cell.id}
                                    className="px-4"
                                ...
                                {row.getVisibleCells().map((cell) => (
                                  <td
                                    ...
                                    onClick={() => {
                                      if (
                                        openDrawerId ==
                                          cell?.row.original[cardFiled?.id] &&
                                        setOpenDrawer &&
                                        !["action", "select", "sno"].includes(
                                          cell.column.id,
                                        )
                ...
                <div className="mb-3 grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-[14px]">
                  {table
                    .getRowModel()
                    .rows.map((v) => v.original)
                    .map((j) => {
                      const active = openDrawerId === j[cardFiled?.id];
                      const m = j?.[cardFiled?.status]
                        ? STATUS_META[
                            j?.[cardFiled?.status]?.toLocaleLowerCase()
                          ]
                        : STATUS_META["success"];
                      ...
                      return (
                        <div
                          key={j.id}
                          onClick={() => {
                            if (cardNavigateEditHandler)
                              cardNavigateEditHandler(j);
                            else if (
                              openDrawerId == j[cardFiled?.id] &&
                              setOpenDrawer
                            )
                              setOpenDrawer(null);
                            else if (setOpenDrawer) setOpenDrawer(j);
                          }}
                          className={`bg-white shadow-lg rounded-[14px] p-[18px] cursor-pointer transition-all duration-[180ms] border-[1.5px] ${
                            active ? "border-accent" : "border-accent-dim"
                          }`}
                        >
                          <div className="flex justify-between items-start mb-3">
                            <div className="w-9 h-9 rounded-[9px] flex items-center justify-center text-base border border-gray-300">
                              {TYPE_ICONS[j.jobType] || "📦"}
                            </div>
                            <Spark
                          ...
                          <div className="font-extrabold text-sm mb-1 tracking-tight">
                            {j?.[cardFiled.title]}
                          </div>
                          <div className="text-xs font-mono mb-3">
                            {j?.[cardFiled.subTitle1]} ·{" "}
                            {j?.[cardFiled.subTitle2]}
                          </div>
                          <div className="flex justify-between items-center text-xs">
                            {j?.[cardFiled?.status] ? (
                              <StatusPill
                                status={j?.[cardFiled?.status] || "Success"}
                              />
                            ) : (
                              <div />
                            )}
                            <span className="font-bold font-mono">
                              {j[cardFiled.assets] > 0
                                ? j[cardFiled.assets] + " assets"
                                : "—"}
                            </span>
                          </div>
                        </div>
      ...
      {enablePagination && data?.length ? (
        <div
          className="flex items-center justify-between px-4 py-2 bg-nav-bg rounded-b-lg"
          style={{ margin: 0 }}
        ...
        >
          ...
          <div className="text-sm text-gray-400 flex items-center gap-1">
            ...
            jobs
            {selectedRows?.length ? (
              <div className="rounded-full h-1 w-1 bg-gray-500 ml-1" />
            ) : (
              ""
            )}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-md border hover:bg-gray-100 disabled:opacity-40"
            >
              ‹
            </button>
            ...
            {pages.map((p, i) =>
              p === "..." ? (
                ...
                <button
                  ...
                  onClick={() => {
                    if (pagination) {
                      onPaginationChange(pagination.pageSize, p);
                    } else {
      ...
      {isAdvanceFilterModalOpen && (
        <AdvanceFilter
          ...
          externalFilters={isDrawer ? drawerFilters : undefined}
          onSetFilters={
            isDrawer ? (filters) => setDrawerFilters(filters) : undefined
          }
          onAddFilter={
            isDrawer
              ? (filter) =>
          ...
          onRemoveFilter={
            isDrawer
              ? (index) =>
                  setDrawerFilters((prev) => prev.filter((_, i) => i !== index))
              : undefined
          }
        />
      )}
      ...
      <Dialog
        ...
        slotProps={{
          paper: {
            className:
              "rounded-2xl overflow-hidden shadow-theme border border-border-theme flex flex-col max-h-[min(90vh,calc(100dvh-48px))]",
            sx: {
              backgroundImage:
                "linear-gradient(180deg, color-mix(in srgb, var(--page-bg) 85%, var(--surface)) 0%, var(--surface) 120px)",
              overflow: "hidden",
              display: "flex",
      ...
      >
        <DialogTitle className="!flex !shrink-0 !flex-row !items-start !justify-between !gap-3 !pr-2 !pb-3 ai-gradient-header text-white shadow-md">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <span className="block text-lg font-semibold tracking-tight leading-tight">
                AI insights
              </span>
              <span className="block text-xs font-normal text-white/85 mt-1 leading-snug">
        ...
        <DialogContent
          dividers
          className="!flex !min-h-0 !flex-1 !flex-col !overflow-hidden !border-border-theme !bg-page-bg/40 !p-0"
        >
          <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-border-theme bg-surface/95 px-3 py-3 shadow-[0_1px_0_rgba(var(--accent-rgb),0.06)] backdrop-blur-sm z-[8]">
            {aiModels.length > 0 && (
            ...
            <div className="flex flex-wrap items-center gap-2 min-w-0 sm:ml-auto">
              <Button
                ...
                sx={{
                  ...
                  boxShadow: "none",
                  "&:hover": {
                    boxShadow: "0 6px 16px rgba(var(--accent-rgb), 0.28)",
                  },
                }}
              >
                Refresh insights
                {queuedKpiRequests.length ? ` (${queuedKpiRequests.length})` : ""}
              </Button>
              <Button
                variant="outlined"
                size="small"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-3 py-3 [scrollbar-gutter:stable]">
            ...
            {!aiError && aiLoading && (
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 rounded-2xl border border-border-theme bg-surface px-5 py-8 text-sm text-text-sub shadow-sm">
                <CircularProgress size={28} sx={{ color: "var(--accent)" }} />
                <div>
                  <div className="font-semibold text-text-primary">Working on your analysis</div>
                  <div className="text-text-sub mt-0.5">{currentAiStageMessage}</div>
                </div>
              </div>
            )}
            {!aiLoading && aiResult && (
            ...
            <div className="mt-6 rounded-2xl border border-border-theme bg-surface p-4 md:p-5 shadow-sm ring-1 ring-[rgba(var(--accent-rgb),0.08)]">
              <div className="flex items-center gap-2 mb-3 text-text-primary">
                <MessageSquareText className="text-accent shrink-0" size={20} strokeWidth={1.75} aria-hidden />
              ...
              <div className="min-h-[300px] max-h-[380px] overflow-y-auto rounded-xl border border-border-theme bg-gradient-to-b from-page-bg/90 to-surface p-4 mb-3 shadow-inner text-sm [scrollbar-width:thin]">
                {chatHistory.length === 0 && (
                  <div className="text-text-sub text-sm py-10 text-center px-4 leading-relaxed max-w-md mx-auto">
                    <span className="block text-accent font-medium mb-1">Start the conversation</span>
                ...
                {chatHistory.map((m, idx) => (
                  <div
                    ...
                    className={`mb-3 last:mb-0 p-3 rounded-xl ${
                      m.role === "user"
                        ? "bg-accent-dim border border-border-theme text-text-primary ml-3 sm:ml-8 shadow-sm"
                        : "bg-surface border border-border-theme text-text-primary mr-3 sm:mr-8 shadow-sm"
                    }`}
                  >
                    {m.role === "user" ? (
                      <>
                        <span className="font-semibold text-[10px] uppercase tracking-wider text-accent block mb-1.5">
                          You
                        </span>
              ...
              <div className="flex gap-2 items-stretch">
                <input
                  type="text"
                  className="flex-1 border border-border-theme rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-[rgba(var(--accent-rgb),0.35)] focus:border-accent outline-none transition bg-surface shadow-sm placeholder:text-text-faint"
                ...
                <Button
                  ...
                  sx={{
                    ...
                    borderRadius: "12px",
                    boxShadow: "none",
                    "&:hover": { boxShadow: "0 6px 16px rgba(var(--accent-rgb), 0.28)" },
                  }}
                >
              ...
              <div className="mt-4 pt-4 border-t border-border-theme">
                <label className="block text-xs font-semibold text-text-sub mb-1.5">Focus columns (optional)</label>
                <Autocomplete
                  multiple
                  size="small"
                  ...
                  sx={{
                    "& .MuiOutlinedInput-root": { py: 0.5, borderRadius: "12px", bgcolor: "var(--surface)" },
                  }}
                />
