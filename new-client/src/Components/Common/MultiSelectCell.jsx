...
const MultiSelectCell = ({
  ...
  return (
    <div className="relative w-40" ref={dropdownRef}>
      ...
      <div
        className={`min-h-[32px] border rounded-lg px-2 py-1 bg-input-bg border-input-bg flex gap-1 items-center flex-wrap ${disabled ? "bg-gray-100 cursor-not-allowed opacity-70" : "cursor-pointer"}`}
        onClick={() => {
      ...
      >
        {selectedValues?.length === 0 ? (
          ...
          selectedValues?.map((value, index) => (
            <span
              key={index}
              className="inline-flex items-center gap-1 bg-accent-dim text-accent text-xs px-2 py-1 rounded"
            >
              {value}
              {!disabled && (
                <button
