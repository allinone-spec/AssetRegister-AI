...
const ChartComponent = ({
  ...
  const [chartHeight, setChartHeight] = useState(isHideDrawer ? 70 : 50);
  ...
  const pieChartOptions = {
    ...
    onClick: (event, elements, chart) => {
      if (elements.length > 0) {
        ...
        if (isHideDrawer) setShowModal(true);
        else setShowParentModal(true);
      }
    },
    layout: {
      padding: {
    ...
    (chartType) => {
      ...
      return {
        ...
        legend: {
          ...
          pageIconColor: isDark ? "#94a3b8" : "#2f4554",
          pageIconInactiveColor: isDark ? "#475569" : "#aaa",
          pageIconSize: 15,
          pageTextStyle: {
            color: isDark ? "#f1f5f9" : "#333",
          },
    ...
    "Matrix Chart":
      ChartselectedTableName?.length > 0 ? (
        <MatrixChart
          ...
          setSelectedSlice={
            isHideDrawer ? setSelectedSlice : setSelectedParentSlice
          }
          setShowModal={isHideDrawer ? setShowModal : setShowParentModal}
  ...
  return (
    <div className="flex w-full flex-col p-4 bg-surface rounded-lg">
            ...
            {permissionList.includes(routeName) &&
              permissionDetails[routeName]?.hasWriteOnly && (
                <HiOutlineDotsVertical
                  ...
                  className="w-5 h-5 text-sub cursor-pointer hover:text-primary transition-colors"
                />
              )}
            {menuOpen && (
              <div className="absolute right-10 mt-2 w-48 bg-surface border border-theme rounded-md shadow-theme z-10">
                <div className="p-2">
                  <label className="flex items-center cursor-pointer">
                    <input
                      ...
                      className="hidden"
                    />
                    <div className="relative w-10 h-5 bg-input-bg rounded-full">
                      <div
                        className={`absolute left-0 top-0 w-5 h-5 bg-accent rounded-full shadow-md transform transition-transform ${
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 bg-surface text-primary font-medium text-base rounded shadow-theme hover:text-accent transition-colors border border-theme"
                >
      ...
      >
        {loading ? (
          <span className="text-sub text-sm">Loading chart...</span>
        ...
        <div className="text-sub mt-3 text-sm z-10">{description}</div>
      </section>
