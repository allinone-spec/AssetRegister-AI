...
/** Per-user prompt overrides (stored encrypted at rest on AI service; user wins over org defaults for that user). */
export const saveUserPromptConfig = async (payload) => {
  const response = await aiApi.put("/prompt-config/user", payload);
  return response.data;
};

/** Per-user API key overrides (optional; falls back to server defaults). */
